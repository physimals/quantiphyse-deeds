"""
Setup script for quantiphyse-deeds
"""
import os
import sys
import subprocess
...
def get_extensions(rootdir):
    ...
    extensions.append(Extension("%s.deeds_wrapper" % MODULE,
                                sources=['%s/deeds_wrapper.pyx' % MODULE,
                                         '%s/src/TMI2013/deedsMSTssc.cpp' % MODULE],
                                include_dirs=[numpy.get_include(), 
                                              "%s/src/TMI2013/" % MODULE, 
                                              extra_inc],
                                language="c++", 
...
kwargs = {
    'name' : 'quantiphyse-deeds',
    'version' : get_version(module_dir),
    'description' : 'Quantiphyse plugin for DEEDS registration method',
